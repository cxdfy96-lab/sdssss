...
async def custom_reset(callback: types.CallbackQuery):
    ...
    await callback.answer("تم الاستعادة")
    await custom_settings(callback)

@dp.message(lambda m: m.text and m.text.startswith("waiting_"))
...
@dp.message(lambda m: m.text and "waiting_for_mute_user_id" in str(m.text))
