...
# ============================================================
# KEYBOARDS (أزرار حقيقية تعمل بفعالية)
# ============================================================

def main_keyboard(user_id: int):
    rows = [
        [
            types.InlineKeyboardButton(text="تفعيل الاشتراك المجاني (شهر)", callback_data="subscription"),
...
async def start(message: types.Message):
    ...
    status_text = "حسابك غير منصب حالياً."
    if row and row.get("session_string"):
        status_text = "حسابك منصب ويعمل بنجاح في الخلفية! ✅"

    await message.answer(
        f"أهلاً بك في بوت إدارة الحساب الذكي.\n\n"
        f"الحالة: {status_text}\n"
        f"الاشتراك المجاني: مفعل لمدة شهر كامل.\n\n"
...
async def main_callback(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "أهلاً بك في القائمة الرئيسية للإدارة.",
...
async def subscription(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        "تم تفعيل اشتراكك المجاني لمدة شهر بنجاح وحفظ حالتك!\n\n"
        "اختر طريقة ربط حسابك:",
        reply_markup=kb,
    )
    await callback.answer()
...
# ============================================================
# LOGIN METHOD 1: PHONE NUMBER & CODE (حقيقي ومحفوظ)
# ============================================================
...
async def finalize_login(client: TelegramClient, user_id: int, message: types.Message):
    ...
    await message.answer(
        f"تم تنصيب الحساب وحفظ الجلسة بنجاح لمدة شهر!\n\nالاسم: {me.first_name or ''}",
        reply_markup=main_keyboard(user_id),
    )
    ...
    async def waiter():
        try:
            ...
            try:
                ...
                print("[DB QR SAVE ERROR]", e)

            await callback.message.answer(
                f"تم تنصيب الحساب وحفظ الجلسة بنجاح عبر QR.\nالاسم: {me.first_name or ''}",
                reply_markup=main_keyboard(user_id),
            )
...
async def instructions(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "تعليمات الاستخدام:\n\n"
        "1. اضغط على تفعيل الاشتراك المجاني لحفظ حسابك لمدة شهر.\n"
        "2. اختر طريقة تسجيل الدخول (برقم الهاتف أو QR).\n"
        "3. سيتم حفظ جلستك في قاعدة البيانات ويعمل اليوزربوت 24/7.",
...
async def mute_panel(callback: types.CallbackQuery):
    await callback.message.edit_text(
        "كتم الأشخاص\n\n"
        "• داخل المحادثة أرسل: كتم / فك كتم\n"
        "• عبر البوت أرسل: كتم الأيدي <الآيدي> / فك كتم الأيدي <الآيدي>",
        reply_markup=back_keyboard(),
    )
    await callback.answer()
...
async def list_words(callback: types.CallbackQuery):
    ...
    await callback.message.answer(text if rows else "لا توجد كلمات محظورة.")
    await callback.answer()


@dp.callback_query(F.data == "lock")
async def lock_panel(callback: types.CallbackQuery):
    await callback.message.edit_text("قفل الخاص يعمل في الخلفية.", reply_markup=back_keyboard())
    await callback.answer()
...
async def notifications(callback: types.CallbackQuery):
    ...
    await callback.answer("تم تبديل حالة الإشعارات.", show_alert=True)
...
async def save_media(callback: types.CallbackQuery):
    ...
    status_msg = "تم تفعيل حفظ الوسائط العادية وذاتية التدمير (TTL)." if not enabled else "تم إيقاف حفظ الوسائط."
...
async def clock_panel(callback: types.CallbackQuery):
    ...
    kb = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [types.InlineKeyboardButton(text="إيقاف الساعة" if enabled else "تفعيل الساعة", callback_data="toggle_clock")],
            [types.InlineKeyboardButton(text="رجوع", callback_data="main")],
        ]
    )
    await callback.message.edit_text(f"الساعة الحية بتوقيت بغداد: {'مفعلة' if enabled else 'متوقفة'}", reply_markup=kb)
    await callback.answer()
...
async def toggle_clock(callback: types.CallbackQuery):
    row = await get_user_row(callback.from_user.id)
    enabled = bool(row.get("clock_enabled", False)) if row else False
    await db_upsert("user_bots", {"user_id": callback.from_user.id, "clock_enabled": not enabled})
...
async def shortcut_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("أرسل النص البديل للاختصار.")
...
async def shortcut_reply(message: types.Message, state: FSMContext):
    data = await state.get_data()
    try:
        await db_insert("shortcuts", {"owner_user_id": message.from_user.id, "shortcut": data["shortcut"], "response": message.text})
        await message.answer("تمت إضافة الاختصار بنجاح.")
    except Exception:
        await message.answer("حدث خطأ أو الاختصار موجود مسبقاً.")
    await state.clear()
...
async def broadcast(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("أرسل الرسالة التي تريد إذاعتها لجميع مستخدمي الخاص:")
    await callback.answer()
...
async def do_broadcast(message: types.Message, state: FSMContext):
    ...
    if not row or not row.get("account_id"):
        await message.answer("لا يوجد حساب مرتبط للإذاعة.")
        await state.clear()
        return

    client = ACTIVE_CLIENTS.get(row["account_id"])
    if not client:
        await message.answer("اليوزربوت غير متصل حالياً.")
        await state.clear()
        return
    ...
    await message.answer(f"تمت الإذاعة إلى {sent} محادثة.")
    await state.clear()
...
async def add_autoreply(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("أرسل كلمة أو جملة التحفيز للرد:")
    await callback.answer()
...
async def autoreply_trigger(message: types.Message, state: FSMContext):
    ...
    await message.answer("أرسل نص الرد التلقائي:")
...
async def autoreply_reply(message: types.Message, state: FSMContext):
    data = await state.get_data()
    try:
        await db_insert("auto_replies", {"owner_user_id": message.from_user.id, "trigger_text": data["trigger"], "reply_text": message.text, "enabled": True})
        await message.answer("تمت إضافة الرد التلقائي بنجاح.")
    except Exception:
...
async def destroy_panel(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("أرسل ثواني تدمير الرسائل الصادرة (أرسل 0 للتعطيل):")
    await callback.answer()
...
async def set_destroy(message: types.Message, state: FSMContext):
    try:
        ...
        await db_upsert("user_bots", {"user_id": message.from_user.id, "destroy_messages_enabled": sec > 0, "destroy_seconds": sec})
        await message.answer("تم حفظ إعداد التدمير.")
    except Exception:
...
async def forced(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("أرسل معرف القناة للاشتراك الإجباري (مثال: @channel):")
    await callback.answer()
...
async def set_forced(message: types.Message, state: FSMContext):
    ...
    await db_upsert("user_bots", {"user_id": message.from_user.id, "forced_subscription_enabled": True, "forced_subscription_channel": ch})
...
async def welcome(callback: types.CallbackQuery):
    await callback.message.edit_text("إعدادات الترحيب التلقائي مفعلة.", reply_markup=back_keyboard())
    await callback.answer()
...
# ============================================================
# USERBOT (يعمل 24/7 مع حفظ الوقتية والكتم والقنوات)
# ============================================================
        ...
        try:
            ...
            if low.startswith("فك كتم الأيدي "):
                try:
                    ...
                    await db_delete("muted_users", owner_user_id=owner_id, muted_user_id=target_id)
            ...
            for item in await get_shortcuts(owner_id):
                ...
                if shortcut and text == shortcut:
                    try:
                        ...
                        pass
                    await client.send_message(event.chat_id, item.get("response", ""))
                    return
...
# ============================================================
# RESTORE SESSIONS (استعادة الجلسات تلقائياً عند الإقلاع)
# ============================================================
    ...
    try:
        ...
        for row in res.data or []:
            if row.get("session_string") and row.get("is_approved"):
                ...
                account_id = row.get("account_id") or user_id
                if user_id:
                    asyncio.create_task(start_userbot(row["session_string"], account_id, user_id))
...
async def dev_panel(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        "لوحة المطورين:\n\n"
        f"• إجمالي الحسابات المسجلة: {len(rows)}\n"
        f"• اليوزربوتات النشطة حالياً: {active}",
        reply_markup=back_keyboard(),
    )
    await callback.answer()
...
async def generic_message(message: types.Message, state: FSMContext):
    ...
    if text.startswith("حذف كلمة "):
        ...
        if word:
            await db_delete("blocked_words", owner_user_id=message.from_user.id, word=word)
            await message.answer("تم حذف الكلمة بنجاح.")
            return

    await message.answer("استخدم /start لفتح القائمة الرئيسية.", reply_markup=main_keyboard(message.from_user.id))
...
async def main():
    print("[INFO] Restoring active accounts from database...")
    await restore_sessions()
    print("[INFO] Bot polling started successfully...")
